...
def bifurcation(G, freespace_mask, voxel_coords, voxel_resolution):
    ...
    if len(selected_voxels) == 0:
        return None, None, None, None, None  # 선택된 voxel이 없으면 반환
    ...
    if previous_node is None:
        return None, None, None, None, None  # 이전 노드가 없으면 반환
    ...
    # avoid_intersection 수행을 위해 새로운 가지의 직경 계산
    parent_diameter = G.edges.get((previous_node, selected_ending_point), {}).get(
        "diameter", 1.0
    )
    new_diameter = parent_diameter * 0.8

    # 최적 node 계산
    new_node_1_pos = avoid_intersection(
        selected_point_pos,
        raw_new_node_1_pos,
        new_diameter,
        voxel_coords,
    ...
    new_node_2_pos = avoid_intersection(
        selected_point_pos,
        raw_new_node_2_pos,
        new_diameter,
        voxel_coords,
    ...
    if new_node_1_pos is None or new_node_2_pos is None:
        return None, None, None, None, None
